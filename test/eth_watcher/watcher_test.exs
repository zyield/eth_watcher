defmodule EthWatcher.WatcherTest do
  ...
  describe "ETH watcher" do
    ...
    test "process_tx/1 processes token tx", %{token_tx: tx, token_block: block} do
      ...
      assert processed_tx.transaction_hash == "0x00d22086d8b84764cd9d400bcd3237d92a192d0ea2ae8bb1d9de25628c3a28e6"
    end
    ...
    test "process_token_tx/2 return correct values for txs", %{token_tx: tx, token_block: block} do
      processed_tx =
        ...
        assert processed_tx.is_token_tx == true
        assert processed_tx.transaction_hash == "0x2ca89c40b72bf8350a5cdec95fe1a41884250614a31bc996c99229a5ab76e8f0"
    end

    test "process_transactions/1 for token transaction", %{ token_block: token_block } do
      txs             = token_block["transactions"]
      processed_txs   = Watcher.process_transactions(txs, token_block["timestamp"]) |> List.first
      processed_tx    = processed_txs |> List.first

      assert Kernel.length(processed_txs)      == 1
      assert is_map(processed_tx.transfer_log) == true
      ...
      assert processed_tx.transaction_hash     == "0xa86a5857260093e6b262feb450b7ae5a1999cf7d476229ac5992dd4bf7b42553"
    end


    test "process_transactions/1 for dai token transaction", %{ dai_block: dai_block } do
      txs             = dai_block["transactions"]
      processed_txs   = Watcher.process_transactions(txs, dai_block["timestamp"]) |> List.first
      processed_tx    = processed_txs |> List.first

      assert Kernel.length(processed_txs)   == 2
      ...
      assert processed_tx.transaction_hash  == "0x2fd2befb20960b4a7b50c3e2df1caf69855cdac469ccdce1791269adcac15bc9"
    end
    ...
    test "process_transactions/1 for eth transaction", %{ eth_block: eth_block } do
      ...
      assert processed_tx[:transaction_hash]  ==  "0x846c342793f8c7ddb2c2cb13f465cb1d11de12d41735971845b5ab6fc8a91c02"
    end
  end
end
